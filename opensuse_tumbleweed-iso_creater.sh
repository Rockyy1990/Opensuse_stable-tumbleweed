...
install_packages() {
    ...
    if ! zypper install -y kiwi-tools; then
        echo "Fehler beim Installieren von kiwi-tools."
        exit 1
    fi
}
...
add_repositories() {
    ...
    zypper ar -f "$OSS_REPO_URL" "openSUSE-Tumbleweed-OSS"
    zypper ar -f "$NON_OSS_REPO_URL" "openSUSE-Tumbleweed-NON-OSS"
    zypper refresh
}
